...
def find_nearest_neighbors(patched_images_train, patched_images_test, k):
    ...
    nbrs = NearestNeighbors(n_neighbors=k, metric='euclidean', algorithm = 'brute', leaf_size=500)
    
    # Обучение модели на массиве патчей тренировочных изображений
    nbrs.fit(patched_images_train_2d)
    
    
    
    # Поиск k ближайших соседей для каждого патча тестовых изображений
    distances, indices = nbrs.kneighbors(patched_images_test_2d)
