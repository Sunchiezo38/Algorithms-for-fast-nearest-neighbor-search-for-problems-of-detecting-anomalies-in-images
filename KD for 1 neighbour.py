...
dir_path = 'D:/MVTEC/carpet/train/good'
...
test_dir_path = 'D:/MVTEC/carpet/test/color'
mask_dir_path = 'D:/MVTEC/carpet/ground_truth/color'
...
# Инициализация модели KD-tree
kdtree_model = KDTree(X_train, leaf_size=500)

# Поиск ближайших соседей для каждого тестового изображения

distances, indices = kdtree_model.query(X_test.flatten().reshape(-1, 16384), k=1)
...
for i in range(len(test_file_list)):
    ...
    #опредление порога
    diff = np.abs(test_image.flatten() - nearest_neighbor.flatten())
    
    threshold = 50 # параметр для сравнения изображений в пикселях
